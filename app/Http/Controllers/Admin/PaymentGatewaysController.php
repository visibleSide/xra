...
class PaymentGatewaysController extends Controller
{
    ...
    public function registerSlugTypes($type) {
        $slug_types = [
            'view' => [
                'remittance-gateway' => [
                    'automatic' => 'automaticRemittanceView',
                    'manual'    => 'manualRemittanceView',
                ],
            ],
            'edit'  => [
                'remittance-gateway' => [
                    'automatic' => 'automaticRemittanceEdit',
                    'manual'    => 'manualRemittanceEdit',
                ],
            ],
            'update'    => [
                'remittance-gateway' => [
                    'automatic' => 'automaticRemittanceUpdate',
                    'manual'    => 'manualRemittanceUpdate',
                ],
            ],
            'store'     => [
                'remittance-gateway' => [
                    'automatic' => 'automaticRemittanceStore',
                    'manual'    => 'manualRemittanceStore',
                ],
            ],
            'create'    => [
                'remittance-gateway'   => [
                    'manual'    => 'manualRemittanceCreate',
                ],
            ],
        ];
     ...
     * @return view
     */
    public function automaticRemittanceView() {
        $page_title = "Automatic Remittance Gateway";
        $payment_gateways = PaymentGateway::remittance()->automatic()->get();
     ...
     * @return view
     */
    public function manualRemittanceView() {
        $page_title = "Manual Remittance Gateway";
        $payment_gateways = PaymentGateway::remittance()->manual()->get();
     ...
     *
     */
    public function automaticRemittanceEdit($alias) {
        $page_title = "Automatic Remittance Gateway Edit";
        $gateway = PaymentGateway::remittance()->gateway($alias)->firstOrFail();
     ...
     * @return view
     */
    public function manualRemittanceEdit($alias) {
        $page_title = "Manual Remiitance Gateway Edit";
        $payment_gateway = PaymentGateway::remittance()->manual()->gateway($alias)->firstOrFail();
     ...
     * @param \Illuminate\Http\Request $request
     */
    public function automaticRemittanceStore(Request $request) {
        ...
        $validator = Validator::make($request->all(),[
            'gateway_name'              => ['required','string','max:60',Rule::unique('payment_gateways','alias')->where(function($query) use ($gateway_name) {
                ...
                $query->where('slug',PaymentGatewayConst::remittance_money_slug())->where('type',PaymentGatewayConst::AUTOMATIC)->where('alias',$alias);
            })],
        ...
        $validated['slug']          = Str::slug("Remittance Gateway");
     ...
     * @param string $alias
     */
    public function automaticRemittanceUpdate(Request $request,$alias) {
        ...
        $gateway = PaymentGateway::remittance()->automatic()->gateway($alias)->first();
        ...
        foreach($validated[$input_field_base_name] ?? [] as $currency => $item) {
            ...
            $item['alias']              = PaymentGatewayConst::remittance_money_slug() . "-" . Str::slug($gateway->name . " " . $currency . " " . $gateway->type);
     ...
     */
    public function manualRemittanceCreate() {
        $page_title = "Manual Remiitance Gateway";
     ...
     * @return view
     */
    public function manualRemittanceStore(Request $request) {
        $gateway_name = $request->gateway_name;
        $validator = Validator::make($request->all(),[
            'gateway_name'          => ['required','string','max:60',Rule::unique('payment_gateways','alias')->where(function($query) use ($gateway_name) {
                $alias = Str::slug($gateway_name);
                $query->where('slug',PaymentGatewayConst::remittance_money_slug())->where('type',PaymentGatewayConst::MANUAL)->where('alias',$alias);
            })],
        ...
        $validator->after(function ($validator) use ($gateway_name) {
            // Search Gateway is unique or not
            if(PaymentGateway::remittance()->manual()->gateway(Str::slug($gateway_name))->exists()) {
                $validator->errors()->add(
                    'gateway_name', 'The gateway name has already been taken.'
                );
            }
        ...
        $validated['alias']                 = Str::slug($validated['gateway_name']);
        $validated['name']                  = $validated['gateway_name'];
        $validated['slug']                  = Str::slug(PaymentGatewayConst::REMITTANCE_MONEY);
        ...
        $currency_validated['alias'] = PaymentGatewayConst::remittance_money_slug() . "-" . Str::slug($currency_validated['name'] . " " . PaymentGatewayConst::MANUAL);
        ...
        return redirect()->route('admin.payment.gateway.view',['remittance-gateway','manual'])->with(['success' => ['Payment gateway added successfully!']]);
     ...
     * @param string $alias
     */
    public function manualRemittanceUpdate(Request $request,$alias) {
        // Find gateway is available or not
        $gateway = PaymentGateway::remittance()->manual()->gateway($alias)->first();
        ...
        $validator = Validator::make($request->all(),[
            'gateway_name'          => ['required','string','max:60',Rule::unique('payment_gateways','alias')->where(function($query) use ($gateway_name, $gateway) {
                $alias = Str::slug($gateway_name);
                $query->whereNot('id',$gateway->id)->where('slug',PaymentGatewayConst::remittance_money_slug())->where('type',PaymentGatewayConst::MANUAL)->where('alias',$alias);
            })],
            'desc'                  => 'nullable|string|max:10000',
            'label'                 => 'nullable|array',
            'label.*'               => 'nullable|string|max:50',
        ...
        $validator->after(function ($validator) use ($gateway_name,$gateway) {
            ...
            if(PaymentGateway::whereNot(function($query) use ($gateway) {
                ...
                $alias = Str::slug($gateway_name);
                $query->where('slug',PaymentGatewayConst::remittance_money_slug())
        ...
        return redirect()->route('admin.payment.gateway.view',['remittance-gateway','manual'])->with(['success' => ['Payment gateway updated successfully!']]);
