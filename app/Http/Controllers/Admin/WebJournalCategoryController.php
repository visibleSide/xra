...
class WebJournalCategoryController extends Controller
{
    /**
    ...
    public function index(){
        $page_title     = "Web Journal Categories";
        $categories     = WebJournalCategory::orderByDesc('id')->get();
        $languages      = Language::get();

        return view('admin.sections.setup-sections.web-journal-category.index',compact(
            'page_title',
            'categories',
            'languages'
        ));
    ...
    public function store(Request $request){
        ...
        if(WebJournalCategory::where('slug',$slug)->exists()) return back()->with(['error' => ['Name is similar. Please update/change this Name']]);
        
      
        try{
            
            WebJournalCategory::updateOrCreate(['slug' => $slug],['name'=> $data]);
        }catch(Exception $e){
    ...
    public function categoryStatusUpdate(Request $request){
        ...
        try {
            $category = WebJournalCategory::find($validated['data_target']);
            if($category) {
                $category->update([
    ...
    public function delete(Request $request){
        ...
        try{
            $category = WebJournalCategory::find($request->target);
            if($category) {
